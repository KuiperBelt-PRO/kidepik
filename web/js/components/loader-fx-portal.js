/**
 * Receta FX para portales mágicos (dolmen v1).
 *
 * Partículas blancas que nacen en las tres rocas y fluyen hacia el vano.
 *
...
const SPAWN_POINTS_UPRIGHT = 5;
const SPAWN_POINTS_LINTEL = 6;
...
export function extractPortalFxAnchors(element) {
  ...
  if (left) {
    ...
    for (let i = 0; i < SPAWN_POINTS_UPRIGHT; i += 1) {
      const t = (i + 0.5) / SPAWN_POINTS_UPRIGHT;
      anchors.push({
        id: `spawn-left-${i}`,
        kind: "point",
        x: b.maxX,
  ...
  if (lintel) {
    ...
    for (let i = 0; i < SPAWN_POINTS_LINTEL; i += 1) {
      const t = (i + 0.5) / SPAWN_POINTS_LINTEL;
      anchors.push({
        id: `spawn-lintel-${i}`,
        kind: "point",
