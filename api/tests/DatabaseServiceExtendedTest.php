<?php

declare(strict_types=1);

namespace Kidepik\Api\Tests;

use Kidepik\Api\Services\DatabaseService;
use Kidepik\Shared\Config;
use Kidepik\Shared\Database\MigrationRunner;
use Kidepik\Shared\Database\PdoFactory;
use PHPUnit\Framework\TestCase;

final class DatabaseServiceExtendedTest extends TestCase
{
    public function testLatestLegalDocumentUnknownSlug(): void
    {
        $url = Config::databaseUrl();
        if ($url === null) {
            self::markTestSkipped('DATABASE_URL no configurada');
        }

        MigrationRunner::fromEnv(dirname(__DIR__, 2))->ensureApplied();

        $db = new DatabaseService();

        self::assertNull($db->latestLegalDocument('not-a-real-slug'));
    }

    public function testLatestLegalDocumentHandlesMissingTable(): void
    {
        $url = Config::databaseUrl();
        if ($url === null) {
            self::markTestSkipped('DATABASE_URL no configurada');
        }

        PdoFactory::resetSharedForTests();
        $pdo = PdoFactory::fromDatabaseUrl($url);
        $pdo->exec('drop table if exists public.legal_documents');

        $db = new DatabaseService();
        self::assertNull($db->latestLegalDocument('terms'));

        TestDbSeeds::restoreLegalDocumentsTable();
        PdoFactory::resetSharedForTests();
    }

    public function testServiceWithoutInjectedDependencies(): void
    {
        $url = Config::databaseUrl();
        if ($url === null) {
            self::markTestSkipped('DATABASE_URL no configurada');
        }

        MigrationRunner::fromEnv(dirname(__DIR__, 2))->ensureApplied();
        PdoFactory::resetSharedForTests();

        $db = new DatabaseService();

        self::assertTrue($db->isReachable());
        self::assertNotNull($db->pocHealthMessage());
    }
}
