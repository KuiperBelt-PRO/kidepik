...
| --- | --- | --- | --- |
| API FastAPI | pytest + pytest-asyncio + httpx + respx | **≥ 85%** | `backend/app/**` |
...
```powershell
docker compose --env-file .env.poc -f docker/compose.yaml exec api pytest -q --cov=app --cov-report=term-missing --cov-fail-under=85
